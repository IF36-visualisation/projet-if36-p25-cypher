...
- Beaucoup de trajets durent moins de 3h, pour ne pas dire la majorité.
...
library(scales)  # Il faut appeler scales si ce n'est pas encore fait

# Comptage des départs
departures <- data_trains %>%
  group_by(Departure_station) %>%
  summarise(nb_departures = sum(Number_of_expected_circulations, na.rm = TRUE))

# Comptage des arrivées
arrivals <- data_trains %>%
  group_by(Arrival_station) %>%
  summarise(nb_arrivals = sum(Number_of_expected_circulations, na.rm = TRUE))

# Fusionner les deux
frequentation <- full_join(departures, arrivals, by = c("Departure_station" = "Arrival_station")) %>%
  mutate(total_freq = rowSums(across(c(nb_departures, nb_arrivals)), na.rm = TRUE)) %>%
  rename(Station = Departure_station) %>%
  arrange(desc(total_freq))


ggplot(frequentation[1:20, ], aes(x = reorder(Station, total_freq), y = total_freq)) +
  geom_bar(stat = "identity", fill = "tomato") +
  scale_y_continuous(labels = comma) +
  coord_flip() +
  labs(title = "Top 10 gares par trajets", x = "Gare", y = "Fréquentation totale (départs + arrivées)")


```
<p align="justify">
A changer : faire par années 
</p>
